...
**なぜ2つに分けるのか。** プロジェクト固有の知識（このアプリの設計判断）と、どこでも使える知識（Deepgram の使い方、Fly.io の罠）は性質が違うからです。前者はリポジトリと一緒に人へ渡すべきで、後者は自分の資産として1箇所に集めたい。詳しくは [5. 構成](05-structure.md) で説明します。
...
回答に**ページ名の引用**（`termlens-architecture.md より` など）が付いていれば、正しく Wiki を読んでいます。付いていなければ、`index.md` から読むよう指示し直してください。
