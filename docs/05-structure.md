...
### `docs/raw/` は git 追跡しない

**`raw/` の中身はローカル限定**にし、`README.md` だけを追跡します。インストーラの `--connect` が `.gitignore` に以下を追加します。
...
1. **raw は定義上「加工しない生資料」**なので統制が効きません。セッション記録・議事録・スクリーンショットには API キーや個人情報が混ざります
2. **一度コミットすると履歴から消えません。** `git rm` しても残り、復旧には `git filter-repo` + force push + キーの再発行が必要です

一方 `docs/wiki/` は**追跡します**。差分レビュー（`git diff docs/wiki`）が運用の要だからです。wiki は LLM が要約して書くので、**その過程が実質的なフィルタになります**。

> 生資料もチームで共有したい場合は、この方針を変えるのではなく**別の private リポジトリに分けます**。公開範囲の異なるものを1つのリポジトリに混ぜないのが原則です。
